...
export default function PingGraph({ hideLabel }: { hideLabel?: boolean }) {
  ...
  }, [Object.keys(knownPeers).length, hideLabel]);
  ...
  for (const key in knownPeers) {
    if (Object.prototype.hasOwnProperty.call(knownPeers, key)) {
      ...
      data.push({
        data: {
          id: peer.peerId,
          label: hideLabel ? undefined : peer.peerId,
          color: online
  ...
  data.push({
    data: {
      id: myPeerId,
      label: hideLabel ? undefined : myPeerId,
  ...
  return (
    ...
    >
      <CytoscapeComponent
        key={String(hideLabel)} //hack to make label hiding work
        elements={elements}
